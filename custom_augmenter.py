...
def augmentation(image_name, image, pink, pix_pink, mask, black, pix_black, width, height):
    ...
    for i in range(2):
        ...
        save_name_img = 'D:/trained_models/Training_Data_With_Augmentation/images/zoom_trans_rot_' + str(i) + '_' + image_name
        pink.save(save_name_img)
        save_name_mask = 'D:/trained_models/Training_Data_With_Augmentation/masks/zoom_trans_rot_mask_' + str(i) + '_' + image_name
        black.save(save_name_mask)

        for x in range(width):
            for y in range(height):
                if black.load()[x,y] != 0 and black.load()[x,y] != 249 and black.load()[x,y] != 250 and black.load()[x,y] != 251 and black.load()[x,y] != 252 and black.load()[x,y] != 253 and black.load()[x,y] != 254 and black.load()[x,y] != 255:
                    print('WARNING!')
